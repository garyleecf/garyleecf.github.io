...
The ITU's framing---*AI and Communication*, not "AI for communication"---is intentional (I believe). The proposition is not that AI will enhance a communication network from the outside. It is that AI becomes part of the design fabric of the network itself (so-called "AI-native"). What does that actually mean in practice?

The short version: some of the assumptions that wireless systems were built on are worth revisiting. They are not wrong exactly, but are perhaps simplifications that kept the mathematics and analysis tractable, and they served the field well. However, at higher frequencies, where a significant share of 5G's and 6G's performance ambitions live, those simplifications start to cost something real and measurable. AI-native design is, in part, a proposal to stop leaving that performance on the table.

---

## Simplification in our comfort zone

When a signal traverse through the air, it doesn't go point to point in a single straight line. It scatters, bounces off buildings, diffracts around corners, reflects off vehicles, gets absorbed by foliage. By the time it arrives at the receiver, it comprises dozens of paths, each at a slightly different angle, different delay, and different amplitude.
...
<figure style="margin: 1.8em 0 1.4em;">
  ...
  <figcaption style="text-align: center; font-size: 0.82em; margin-top: 0.5em;">A signal traveling through a real environment does not take a single path. Scattering, reflection, and diffraction produce a superposition of many arrivals at the receiver.</figcaption>
</figure>

Characterising that precisely is, in general, very hard. So classical theory made a practical decision: if you have many independent random contributions summing together, invoke the central limit theorem, and the aggregate looks Gaussian, i.e., white noise, Rayleigh fading. A handful of parameters with tractable models.

This is an impressive line of reasoning, and it has been extraordinarily productive. Shannon capacity, optimal receiver design, error-correcting codes---most of what we actually deploy runs on Gaussian foundations.

But it is still an approximation. And as we push into higher frequencies like millimetre-wave and upper mid-bands (in line with 6G's ambition for high throughputs), that approximation starts to cost us something real.

---

## The channel that gets simpler and stranger at the same time

Here is something that might surprise people when they first encounter it. You might expect that going to higher frequencies makes the wireless channel more chaotic. More reflections, more unpredictability. In fact, it goes the other way.

At higher frequencies (millimetre-wave for instance), signals attenuate aggressively. Most of those scattered bounces just disappear. What you are left with is a small number of dominant propagation paths: a line-of-sight component if you are lucky, a strong reflection off a nearby surface, and then basically nothing else (at least not discernible from the background noise). The channel is *sparse* in the angular domain. Energy arrives from a few directions, not all of them.
...
<figure style="margin: 1.8em 0 1.4em;">
  ...
  <figcaption style="text-align: center; font-size: 0.82em; margin-top: 0.5em;">At sub-6GHz frequencies, energy arrives from many directions simultaneously (left). At millimetre-wave, higher path loss strips away most reflections, leaving only a handful of dominant propagation paths (right).</figcaption>
</figure>
...
The performance cost shows up most clearly in how the network designs its *codebook*, i.e., the set of beams it uses to serve users across a cell.
...
The uninformed approach is to spread those beams uniformly across all possible angles. If a user could be *anywhere*, cover everything equally. That is fig. 3a: several beams, evenly spaced, no assumptions made about the world.

Now add the environment. Users are in buildings, on streets, in corridors. The geometry constrains where they actually are. Some angular directions from any given base station are simply not accessible---blocked by a wall, a building, a car park. That is fig. 3b. The codebook has not changed, but now it is obvious that some beams are pointing at regions where no user will ever be. Those are wasted degrees of freedom, since they are capacity you provisioned for but cannot use.
...
If you know the channel is sparse, you can exploit it directly. Compressed sensing algorithms (e.g., matching pursuit, LASSO) recover sparse signals from far fewer measurements than a Gaussian estimator would require. You don't need to probe every direction if you know most directions are empty. This idea has been influential in 5G channel estimation research [2].

But compressed sensing requires you to specify the sparsity structure: which basis the channel is sparse in, how sparse, what the noise model looks like. Get those wrong and the gains disappear. Real millimetre-wave channels are sparse in ways that are approximately but not exactly what a textbook model says---the support varies with environment, there are correlations across antenna arrays that the standard basis ignores.
...
<figure style="margin: 1.8em 0 1.4em;">
  ...
  <figcaption style="text-align: center; font-size: 0.82em; margin-top: 0.5em;">The classical Gaussian assumption is baked into the lower layers of the stack---pilot structures, feedback formats, link adaptation curves. An AI-native system redesigns those interfaces, not just the algorithms running above them.</figcaption>
</figure>
